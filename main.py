...
local_server = True
app = Flask(__name__)
app.config['UPLOAD_FOLDER']=params['upload_loction']
...
class Posts(db.Model):
    ...
    semi_title = db.Column(db.String(80), unique=False, nullable=False)
    slug = db.Column(db.String(21), unique=True, nullable=False)
    post_by = db.Column(db.String(21), unique=False, nullable=False)
...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_user']) :
        if request.method == 'POST' :
            ...
            semi_title=request.form.get('semi_title')
            ...
            date=datetime.now()
            if sno=='0':
                post=Posts(title=title,semi_title=semi_title,slug=slug,post_by=post_by,content=content,img_file=img_file,date=date)
                db.session.add(post)
                db.session.commit()
            else:
                post=Posts.query.filter_by(sno=sno).first()
                post.title = title
                post.semi_title = semi_title
                post.slug = slug
